    ...
    >
      ...
      {Object.entries(data).map(([key, value], index) => {
        ...
        if (value === null || value === undefined || (Array.isArray(value) && value.length === 0)) {
          return (
            <div key={key} className="flex flex-col gap-1 border-t pt-4 first:border-none">
              <div className="flex items-center gap-2">
                <Info className="w-4 h-4 text-muted-foreground" />
                <Label className="text-sm font-medium">{formattedKey}</Label>
              </div>
              <Input value="N/A" disabled className="w-full opacity-70 text-muted-foreground" />
            </div>
          );
        }
        ...
        if (Array.isArray(value) && value.every((val) => typeof val === "object")) {
          return (
            <div key={key} className="flex flex-col gap-2 border-t pt-4 first:border-none">
              <div className="flex items-center gap-2">
                <Info className="w-4 h-4 text-muted-foreground" />
                <Label className="text-sm font-medium">{formattedKey}</Label>
              </div>
        ...
        return (
          <div key={key} className="flex flex-col gap-1 border-t pt-4 first:border-none">
            ...
            <Input value={String(value)} disabled className="w-full text-right opacity-90" />
          </div>
        );
      })}
    </NodeToolbar>
