...
export const generateDiagram = async (
    ...
    try {
        const diagramResponse = await fetchWithAuth(import.meta.env.VITE_API_BASE_URL + '/ai/generate-diagram', token, {
            method: 'POST',
            headers: {
...
export const getDiagrams = async (
    ...
    try {
        const response = await fetch(import.meta.env.VITE_API_BASE_URL + '/data/get-all-diagrams', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
export const generateModel = async (
    ...
    try {
        const diagramResponse = await fetchWithAuth(import.meta.env.VITE_API_BASE_URL + '/ai/generate-model', token, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json', // Important pour que le serveur sache que les données sont en JSON
            },
            body: JSON.stringify({ modelName: payload.modelName, userPrompt: payload.prompt, previousModelsCode: payload.previousCodes, modelType: payload.modelType }),
        });
        const diagramData = await diagramResponse.json();
...
export const deleteDiagram = async (
    ...
    try {
        const response = await fetchWithAuth(import.meta.env.VITE_API_BASE_URL + `/data/delete-diagram/${diagramId}`, token, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            ...
            toast({
                description: data.error || 'An error occurred while deleting the diagram.',
                variant: 'destructive',
            });
        }
    } catch (error) {
