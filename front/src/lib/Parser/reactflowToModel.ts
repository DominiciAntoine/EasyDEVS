...
const cleanHandleId = (str: string | undefined | null) => str?.replace(/^(in-internal-|out-internal-|out-|in-)/, '');
    ...
    const internalPorts: string[] = nodesAndEdges.nodes
        ...
        const directInternalEdges = nodesAndEdges.edges.filter((anEdge) => {
          ...
          if (cleanSource && cleanTarget) {
            return internalPorts.includes(cleanSource) && internalPorts.includes(cleanTarget)
          }
...
const getModelPorts = (
    ...
    const portIn =
        node.data.inputPorts?.map<components["schemas"]["json.ModelPort"]>(
            (p) => ({ id: p.id, type: "in" })
        ) ?? [];
    const portOut =
...
const nodeToModel = (
    ...
    return {
        ...
        libId: undefined,
        connections: node.data.modelType === 'coupled' ? getModelConnection(node, nodesAndEdges, comp) : [],
    };
};
